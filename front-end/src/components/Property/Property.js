...
const Property = () => {
  ...
  useEffect(() => {
    ...
    if (propertyId) {
      axios
        .get(
          `http://localhost:3000/api/properties/property?propId=${propertyId}`
        )
...
//   useEffect= () => {
//     axios.get(`http://localhost:3000/api/reservations/getDatesList?propId=${propertyId}`)
...
//                   datesDisabled = datesDisabled.concat(
//                     getDates(data[i].startDate, data[i].endDate)
//                 );
//                 }
...
//   }
  ...
  const getBookingPrice = () => {
    ...
    if (guestsCount >= 1) {
      bookingPrice =
        Math.ceil(guestsCount / 2) *
        property.price.base_fee *
        selectedDatesCount;
    }
    return bookingPrice;
  };
  ...
  const handleClick = async () => {
    ...
    await axios
      .post(`http://localhost:3000/api/reservations/`, requestBody)
      .then(function (response) {
        // handle success
        if (response.status == 200) {
          console.log("your reservation is succesfull");
  ...
  return (
    <div className="app-body">
      {property.propId && (
        <div>
          <div
            className="property d-flex flex-row justify-content-center m-lg-5 m-2 flex-lg-nowrap flex-wrap"
            style={{ gap: "10px" }}
          >
            <div className="col-12 col-lg-6">
              ...
              <div className="mt-3">
                ...
                <span className="right" style={{ jusitfyContent: "right" }}>
                  {" "}
              ...
              <div className="mt-3">
                ...
                <ul>
                  {property.houseRules &&
                    ...
                    property.houseRules.map((rule, ruleIndex) => {
                      return <li>{rule} </li>;
                    })}
                </ul>
              </div>
              <div className="mt-3">
                <div>Amenities: </div>
                {property?.ameneties?.airConditioning && (
                  <span className="ms-3">Air Conditioning</span>
                )}
                {property.ameneties?.hairDryer && (
                  <span className="ms-3">Hair Dryer</span>
                )}
                {property.ameneties?.hotTub && (
                  <span className="ms-3">Hot Tub</span>
                )}
                {property.ameneties?.wifi && <span className="ms-3">Wifi</span>}
                {property.ameneties?.iron && <span className="ms-3">Iron </span>}
                {property.ameneties?.washer && (
                  <span className="ms-3">Washer/ Dryer</span>
                )}
              </div>
            </div>
            <div
              id="carouselExampleIndicators"
              class="carousel slide w-100 property-carousel m-3 m-lg-0"
            ...
            >
              ...
              <div
                class="carousel-inner property-carousel"
              ...
              <div class="card-body d-flex flex-column">
                ...
                <div className="d-flex flex-row justify-content-between ">
                  <div className="mt-2">
                    ...
                    <span>
                      {property.price.base_fee} * {selectedDatesCount} night(s)
                      * {Math.ceil(guestsCount / 2)} room(s) ={" "}
                      {getBookingPrice()}
                    </span>
                  </div>
                </div>
          ...
          <div className="property-review d-flex flex-row justify-content-center flex-column mb-5">
            <div class="container user-review-container ">
              <div
                className="d-flex mb-lg-5 m-2 justify-content-between"
                style={{ gap: "10px" }}
              >
                <div className="d-flex" style={{ gap: "10px" }}>
                  ...
                  <b>{property.AverageRatings}</b> <span>*</span>
                  <b>{property.userReview.length} reviews</b>
                </div>
                <div className="d-flex" style={{ gap: "10px" }}>
                  <button
                    type="button"
                  ...
                  return (
                    <div key={index} className="col-sm-6">
                      <p>
                        ...
                        >
                          {" "}
                          {formatDate(review.commentedDate)}{" "}
                        </span>
                      </p>
                      <p>{review.comments}</p>
                    </div>
          ...
          <CommentModal
            ...
            updateComments={updateComments.bind(this)}
          />
        </div>
      )}
    </div>
